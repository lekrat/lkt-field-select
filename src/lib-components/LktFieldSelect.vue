<script lang="ts" setup>
    ...
    // Emits
    const emits = defineEmits(['update:modelValue', 'update:picked', 'click-ui', 'selected-option', 'results']);

    // Props
    const props = withDefaults(defineProps<{
        modelValue: string | number | Option[]
        picked: Option | Option[] | undefined
        class: string
        placeholder: string
        label: string
        palette: string
    ...
    watch(pickedData, (v) => emits('update:picked', v))
    ...
    >
        ...
        <div v-if="!editable && !multiple" class="lkt-field-select__read">
            ...
            <component v-else-if="hasCustomResourceValueSlot" v-bind:is="customResourceValueSlot"
                       v-bind:option="selectedOption"/>
            <template v-else>
                <div class="lkt-field-select__read-value" :class="'lkt-field-option option-' + selectedOption.value"
                     v-html="computedValueText" :title="computedValueText"></div>
            </template>
        ...
        <div v-if="!editable && multiple" class="lkt-field-select__read-multiple">
            ...
            <ul v-else :class="multipleValuesClasses">
                <li v-for="opt in computedValueTexts" :title="opt.label"
                    :class="'lkt-field-option option-' + opt.value">
                    ...
                    <component v-else-if="hasCustomResourceValueSlot" v-bind:is="customResourceValueSlot"
                               v-bind:option="opt" v-bind:data="slotData"/>
                    <template v-else>
                        <div class="lkt-field-select__read-value" v-html="opt.label"></div>
                    </template>
                </li>
